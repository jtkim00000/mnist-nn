...
## Backpropagation
Backpropagation is the process of finding values to set those weights and biases to such that the NN can actually recognize digits. In general backpropagation requires relatively good knowledge of multivariable calculus and processes like gradient descent, so if anything is unclear, please refer to 3b1b's course on NNs. 
